        ...
        if (hasPerms) {
          if (!Object.keys(thisServer).length) {
                ...
                fields: [
                  {
                    ...
                    value:
                      "It looks like this server is in **" +
                      msg.guild.region +
            ...
            msg.guild.fetchMember(bot.user).then(function (member) {
              member.setNickname(
                moment().tz(thisServer.zone).format(thisServer.format)
              );
            });
          } else {
            errorResponse(
              "",
              "Cannot add server to database.",
              "Server already exists, so this command never has to be run again."
            );
          }
        } else {
          errorResponse("set");
      ...
      case "format":
        if (hasPerms) {
          ...
          if (format.length && Object.keys(thisServer).length) {
            ...
            msg.channel.send(
              "Current time format (& help info): " + thisServer.format
            );
          } else {
      ...
      case "help":
        msg.channel.send(
          new Discord.RichEmbed({
            ...
            url: "",
            description:
              "Below you will find all the details on how to get this bot up and running on your server.",
            fields: [
              {
              ...
              },
              {
                ...
                value:
                  "Nice! You can't just yet - the code is being perfected. But soon.",
              },
            ],
          })
        );
            ...
            fields: [
              {
                name:
                  "Preview: " +
                  moment().tz(defaultConfig.zone).format(defaultConfig.format),
                value:
                  "**Timezone**: `" +
                  defaultConfig.zone +
                  "`" +
                  ...
                  "\n" +
                  "**Regions**: `" +
                  JSON.stringify(defaultConfig.zones, null, 2) +
                  "`" +
                  "\n",
              },
            ],
            ...
            fields: [
              {
                ...
                value:
                  "It is currently " +
                  moment().tz(command[2]).format("h:mm A [on] dddd"),
              },
            ],
          })
        );
