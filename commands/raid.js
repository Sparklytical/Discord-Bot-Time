...
module.exports.run = function (msg) {
  ...
  let rsvpTitle = "Ready Check by EvilSpark";
  let senderUserTag =
  ...
  let joinRaid = function () {
    if (
      Object.keys(raidDetails).length &&
      raidDetails.raiders.indexOf(senderUserTag) === -1
    ) {
      // Not raider
    ...
    switch (command[0]) {
      case botConfig.prefix + "raid":
        ...
        if (Object.keys(raidDetails).length) {
          raiderCount = 0;
          if (
            raidDetails.raiders.indexOf(",") === -1 &&
            raidDetails.raiders.length
          ) {
            raiderCount = 1;
          } else if (raidDetails.raiders.indexOf(",") !== -1) {
            raiderCount = raidDetails.raiders.split(",").length;
          ...
          raidSummary =
            ...
            raiderCount +
            " " +
            (raiderCount !== 1 ? "raiders have" : "raider has") +
            " signed up." +
            "\n\n" +
            ...
            "_" +
            "Organized by " +
            raidDetails.createdBy.split("#")[0] +
            "_" +
            (hasPerms && raidDetails.modifiedBy.trim().length
              ? "\n_Last modified by " +
                raidDetails.modifiedBy.split("#")[0] +
                "_"
              : "");
        }
        msg.channel.send(
            ...
            fields: [
              {
                ...
                value:
                  ...
                  "\n" +
                  (hasPerms
                    ? "`!raid new [description]` Prepare for a new raid - description can be any length, and should contain time/date" +
                      "\n"
                    : "") +
                  (hasPerms
                    ? "`!raid update [description]` Modify the name of the current raid without removing raiders" +
                      "\n"
                    : "") +
                  (hasPerms
                    ? "`!raid list` List users that have `!join`ed (without pinging them)" +
                      "\n"
                    : "") +
                  (hasPerms
                    ? "`!raid notify` Ping all users that have `join`ed" + "\n"
                    : "") +
                  (hasPerms
                    ? "`!raid clear [notify]` Remove the latest raid and all that have `join`ed - add `yes` on the end to notify them"
                    : ""),
              },
              {
      ...
      switch (command[1]) {
        ...
        case "set":
          if (!Object.keys(raidDetails).length) {
            ...
            if (desc.length) {
              ...
              storage.setItemSync(raidName, raidDetails);
              msg.channel.send(
                "New raid has been scheduled! **Raiders**, add yourselves using `!join`"
              );
            } else {
              msg.channel.send(
                "You need to give the raid a name/description (and a time, if you so choose)."
              );
            }
          } else {
            msg.channel.send(
              "Sorry, but there's already a raid scheduled. If needed, you can rename it using `!raid update [description]`."
            );
          }
          break;
        case "update":
        ...
        case "reschedule":
          if (Object.keys(raidDetails).length && command.length > 2) {
            ...
            if (desc.length) {
              ...
              let raiderNote =
                raiderCount > 0
                  ? "Might be a good idea to `!raid notify` existing raiders of the change. "
                  : "";
              msg.channel.send(
                "Raid description updated! " +
                  raiderNote +
                  "**Raiders**, add yourselves using `!join`"
              );
            }
          } else if (Object.keys(raidDetails).length) {
            // Needs a new name
            msg.channel.send(
              "You forgot to provide a new name/description (and a time, if you so choose)."
            );
          } else {
            // No event to rename
            msg.channel.send(
              "Sorry, but there's currently no raid scheduled. You can create one using `!raid new [description]`."
            );
          }
        ...
        case "list":
          if (Object.keys(raidDetails).length && raidDetails.raiders.length) {
            ...
            msg.channel.send(
              "Currently there " +
                (raiderCount === 1 ? "is" : "are") +
                " " +
                raidDetails.raiders.split(",").length +
                " raider" +
                (raiderCount === 1 ? "" : "s") +
                ", listed below.\n\n" +
                raiderList
            ...
            msg.channel.send(
              "As there is no raid scheduled, there are no raiders."
            );
          }
          break;
        case "notify":
          ...
          if (
            ...
            msg.channel.send(
              mentions +
                ...
                "**" +
                msg.member.displayName +
                "** has deleted the raid you signed up for."
            );
          }
          raidDetails = {};
          storage.setItemSync(raidName, raidDetails);
